<div>
    ...
    @if ($meeting->attendants->isEmpty() && ! $existingGroups->isEmpty())
        <div class="mt-12">
            <x-input-label :value="__('Load group')" />
            <div class="flex items-center gap-2" title='{{ __('Load group') }}'>
                <x-icons.user-group />
                <x-select wire:model='selectedGroup' class="block w-full mt-1 empty:text-red-500" title="">
                    <option value="" disabled>{{ __('Load group') }}</option>
    ...
    @if (! $meeting->attendants->isEmpty())
        <div class="mt-12">
            <div class="flex items-center gap-2 mt-6" title='{{ __('Save as group') }}'>
                <x-icons.user-group />
                <x-text-input wire:model='groupName'
                    wire:keydown.prevent.enter='storeGroup'
                    class="block w-full mt-1" :placeholder="__('Save as group')" title="" />
